            ...
            for (int i = 0; i < filters.Length; i++) {
                ...
                if (curRenderer != null && curRenderer.enabled && instance.mesh != null) {
                    ...
                    for (int m = 0; m < materials.Length; m++) {
                        ...
                        Debug.Log("allMeshesAndMaterials:"+ allMeshesAndMaterials);
                        Debug.Log("materials ["+m+"]:" + materials[m]);
                        if (materials[m] && !allMeshesAndMaterials.ContainsKey(materials [m].shader.ToString())) {
                            allMeshesAndMaterials.Add(materials [m].shader.ToString(), new Dictionary<Material, List<MeshCombineUtility.MeshInstance>>());
                        }

                        if (materials[m] && !allMeshesAndMaterials [materials [m].shader.ToString()].ContainsKey(materials [m])) {
