...
public class GvrReticlePointer : GvrBasePointer {
    public GameObject myRadialLoader;
    public GameObject expand;
    public GameObject collapse;
    public GameObject teleport;
    private Vector3 myRadialLoaderDefaultScale = new Vector3(0.005f, 0.005f, 1);
  ...
  protected override void Start () {
    base.Start();
        ...
        if (myRadialLoader != null)
        {
            //deactivate the original reticle
            GetComponent<MeshRenderer>().enabled = false;
         //   EventManager.StartListening(EventManager.GAZE_WAIT_STARTED, startWaiting);
         //   EventManager.StartListening(EventManager.GAZE_STOPPED, stopWaiting);
        }
    }
    ...
    private void stopWaiting()
    {
        ...
        startTime = 0;
        if (myRadialLoader != null)
            myRadialLoader.transform.localScale = myRadialLoaderDefaultScale;
    }
    ...
    void Update() {
        ...
        if (isWaiting)
        {
            if(Time.time < startTime + gazeTime) {
                if (myRadialLoader != null)
                    myRadialLoader.transform.localScale += new Vector3(0.014f * Time.deltaTime, 0.014f * Time.deltaTime, 0);
            }
            else
            {
                stopWaiting();
                EventManager.TriggerEvent(EventManager.GAZE_COMPLETED);
            }
        }
    }
  ...
  public override void OnPointerEnter(GameObject targetObject, Vector3 intersectionPosition,
     Ray intersectionRay, bool isInteractive) {
        ...
        startWaiting();
        ExpandableCollapsabe expandableCollapsabe = targetObject.GetComponent<ExpandableCollapsabe>();
        if (expandableCollapsabe)
        {
            if (expandableCollapsabe.isCollapsed && !expandableCollapsabe.isExpaneded)
            {
                expand.SetActive(true);
                collapse.SetActive(false);
            }
            else if (!expandableCollapsabe.isCollapsed && expandableCollapsabe.isExpaneded)
            {
                expand.SetActive(false);
                collapse.SetActive(true);
            }
        }
        Waypoint waypoint = targetObject.GetComponent<Waypoint>();
        if (waypoint)
        {
            teleport.SetActive(true);
        }
        WaypointFoot waypointFoot = targetObject.GetComponent<WaypointFoot>();
        if (waypointFoot)
        {
            teleport.SetActive(true);
        }
